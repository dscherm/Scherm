...
from config import WORKFLOWS, CHECKPOINTS
...
def call_tool(tool_name: str, args: dict) -> dict:
    ...
    if tool_name == "analyze_prompt":
                ...
                if "images" in node_output:
                    for img in node_output["images"]:
                        output_files.append({
                            ...
                            "path": f"C:/ComfyUI/output/{img.get('subfolder', '')}/{img.get('filename', '')}".replace("//", "/")
                        })
                if "gltfFilename" in node_output:
                    output_files.append({
                        "type": "3d_model",
                        "filename": node_output["gltfFilename"],
                        "path": f"C:/ComfyUI/output/{node_output['gltfFilename']}"
                    })
                if "video" in node_output:
                    for vid in node_output["video"]:
                        output_files.append({
                            ...
                            "path": f"C:/ComfyUI/output/{vid.get('filename', '')}"
                        })
        ...
        comfyui_ok = comfyui_api.check_connection()
