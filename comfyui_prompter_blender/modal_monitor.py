# modal_monitor.py - Async job polling with timer modal
...
class COMFYUI_OT_monitor_job(Operator):
            ...
            if success:
                ...
                if status == 'completed':
                    ...
                    if output_path and props.auto_import:
                        self.import_glb(context, output_path)
    ...
    def import_glb(self, context, filepath):
...
def register():
    bpy.utils.register_class(COMFYUI_OT_monitor_job)
    bpy.utils.register_class(COMFYUI_OT_cancel_job)


def unregister():
    bpy.utils.unregister_class(COMFYUI_OT_cancel_job)
    bpy.utils.unregister_class(COMFYUI_OT_monitor_job)
