...
<body>
  ...
  <\/script>
    ...
    `},$=()=>{if(S(!1),P.current){const z=P.current.contentDocument||P.current.contentWindow.document;z.open(),z.write('<html><body style="background:#1a1a2e;color:#888;padding:20px;">Click "Run Code" to see your output</body></html>'),z.close()}},te=()=>{c(o.starterCode||""),p(o.serverCode||""),$()},O=z=>{v.includes(z)||j([...v,z])},U=()=>{I(!0),c(o.solutionCode||"")},G=()=>{i||(r&&r({exerciseId:o.id,answer:l,isCorrect:!0,exerciseType:"data-apis",exerciseTitle:o.title}),s(o.id,o.points))},W=z=>`difficulty difficulty-${z.toLowerCase()}`,K=z=>{navigator.clipboard.writeText(z).then(()=>{alert("Code copied to clipboard!")})},Y=(z,X=!1)=>{const M={"(":")","[":"]","{":"}",'"':'"',"'":"'","`":"`"},B=X?h:l,Q=X?p:c,ne=z.target,ie=ne.selectionStart,re=ne.selectionEnd,me=z.key;if(M[me]){z.preventDefault();const pe=B.substring(ie,re),le=M[me],We=B.substring(0,ie)+me+pe+le+B.substring(re);Q(We),setTimeout(()=>{ne.selectionStart=ne.selectionEnd=ie+1+pe.length},0)}if(z.key==="Enter"){const pe=B[ie-1],le=B[ie],We=B.lastIndexOf(`
